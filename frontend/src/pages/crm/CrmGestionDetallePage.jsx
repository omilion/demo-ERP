...
export default function CrmGestionDetallePage() {
  ...
  const hasPrintableQuote = Boolean(crm.cotizacionComercial || crm.cotizacionLicitacion || crm.ordenCompraOnline)
  const printQuote = () => {
    const printWindow = window.open(`${window.location.origin}/crm/${crm.id}/cotizacion/imprimir`, '_blank')
  ...
  return <main className="page page-wide"><PageHeader title={`Gestión CRM · ${crm.nombre || crm.rsocial || crm.email || `#${crm.id}`}`} subtitle={`Cotización #${crm.ncotizacion || crm.id} · ${crm.ejecutiva || 'Sin ejecutivo asignado'}`} breadcrumb={['Inicio', 'Ventas', 'CRM', 'Gestión y detalle']} actions={<div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>{hasPrintableQuote && <Btn variant="primary" size="sm" icon="printer" onClick={printQuote}>Descargar / imprimir PDF</Btn>}<div style={{ padding: '7px 11px', background: '#f8fafc', border: '1px solid var(--border)', borderRadius: 7, textAlign: 'left' }}><div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase', fontWeight: 700 }}>Última gestión / cambio</div><div style={{ fontSize: 12, fontWeight: 650, marginTop: 2 }}>{latestDate ? new Date(latestDate).toLocaleString('es-CL') : 'Sin registros'}</div></div><Btn variant="secondary" size="sm" onClick={() => navigate('/crm')}>← Volver al CRM</Btn></div>} />
    <CrmOrdenBanner item={crm} detalle={crm} style={{ borderRadius: 10, marginBottom: 18, border: '1px solid var(--border)' }} />
