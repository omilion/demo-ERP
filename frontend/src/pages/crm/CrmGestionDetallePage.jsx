...
import { useCrmDetalle, useCrmGestionCreate, useCrmPatch, useCrmTransicion } from '../../api/crm'
...
function ProductDetail({ oc, cotizacion, items, setItems }) {
  ...
  if (!oc && !cotizacion) return <Section title="Cotización"><div style={{ color: 'var(--text-3)', fontSize: 13 }}>Este registro no tiene una cotización vinculada.</div></Section>
...
export default function CrmGestionDetallePage() {
  ...
  const quoteAction = !crm.esHistorico && !crm.ordenId
    ? ({ LICITACION: { label: 'Crear cotizacion de Licitacion', path: `/crm/nueva/licitacion` } }[crm.canalVenta] || null)
    : null
  ...
  return <main className="page page-wide"><PageHeader title={`Gestión CRM · ${crm.nombre || crm.rsocial || crm.email || `#${crm.id}`}`} subtitle={`Cotización #${crm.ncotizacion || crm.id} · ${crm.ejecutiva || 'Sin ejecutivo asignado'}`} breadcrumb={['Inicio', 'Ventas', 'CRM', 'Gestión y detalle']} actions={<div style={{ display: 'flex', alignItems: 'center', gap: 14 }}><div style={{ padding: '7px 11px', background: '#f8fafc', border: '1px solid var(--border)', borderRadius: 7, textAlign: 'left' }}><div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase', fontWeight: 700 }}>Última gestión / cambio</div><div style={{ fontSize: 12, fontWeight: 650, marginTop: 2 }}>{latestDate ? new Date(latestDate).toLocaleString('es-CL') : 'Sin registros'}</div></div><Btn variant="secondary" size="sm" onClick={() => navigate('/crm')}>← Volver al CRM</Btn></div>} />
    {quoteAction && <section style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: 18, padding: '13px 16px', border: '1px solid #bbf7d0', background: '#f0fdf4', borderRadius: 10 }}><div><strong style={{ fontSize: 14 }}>Cotizacion comercial</strong><div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 3 }}>Abrira el formato real de {quoteAction.label.replace('Crear cotizacion ', '')} y vinculara su orden al CRM.</div></div><Btn variant="primary" size="sm" onClick={() => navigate(quoteAction.path)}>{quoteAction.label}</Btn></section>}
