...
function ProductDetail({ oc }) {
  ...
  return <Section title="Cotización y detalle de productos" tint="#f0fdf4">
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 14 }}><div><div style={{ fontFamily: "'DM Mono', monospace", fontWeight: 750 }}>OC Online #{oc.nCompra}</div><div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 3 }}>{products.length} ítems · {oc.estadoCompra || 'Sin estado'}</div></div><div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase' }}>Total cotizado</div><div style={{ fontSize: 20, fontWeight: 800, color: 'var(--green-800)' }}>{money(oc.total)}</div></div></div>
