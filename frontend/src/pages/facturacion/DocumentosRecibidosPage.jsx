...
export default function DocumentosRecibidosPage() {
  ...
  const openPdf = async id => {
    ...
    try {
      const response = await api.get(`/facturacion/recibidos/${id}/pdf`, { responseType: 'blob' })
      popup.location.href = URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }))
    } catch (error) { popup.close(); toast.error(errorText(error)) }
  }
