...
export default function DocumentosRecibidosPage() {
  ...
  const columns = [
    ...
    { key: 'estado', label: 'Estado', render: value => <Badge tone={value === 'visto' ? 'blue' : 'amber'}>{value}</Badge> },
  ]
  return <main className="page page-wide">
    ...
    {selected && <aside style={{ position: 'fixed', inset: 0, zIndex: 600, display: 'flex', justifyContent: 'flex-end' }}><div onClick={() => setSelected(null)} style={{ position: 'absolute', inset: 0, background: 'oklch(0 0 0 / .38)' }} /><div style={{ position: 'relative', width: 480, maxWidth: '100%', padding: 22, background: '#fff', boxShadow: '-8px 0 48px oklch(0 0 0 / .14)' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 18 }}><strong>Documento recibido</strong><button onClick={() => setSelected(null)}>×</button></div><div style={{ display: 'grid', gap: 10, fontSize: 13 }}><div><b>Emisor:</b> {selected.razonSocialEmisor || '—'}</div><div><b>RUT:</b> {selected.rutEmisor || '—'}</div><div><b>Correo:</b> {selected.remitente || '—'}</div><div><b>Asunto:</b> {selected.asunto || '—'}</div><div><b>Archivo:</b> {selected.archivoNombre || '—'}</div></div><div style={{ display: 'flex', gap: 8, marginTop: 22 }}><Btn variant="secondary" icon="download" onClick={() => downloadXml(selected)}>Descargar XML</Btn><Btn variant="primary" icon="eye" onClick={() => openHtml(selected.id)}>Ver documento</Btn></div></div></aside>}
  </main>
}
