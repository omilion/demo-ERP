...
function ProductoSearch({ onAdd }) {
  ...
  const { data } = useProductos(query.length >= 2 ? { search: query } : {})
  ...
  const select = producto => { onAdd(producto); setQuery(''); setOpen(false); inputRef.current?.focus() }
  return <div ref={boxRef} style={{ position: 'relative' }}>
    <div style={{ position: 'relative' }}><span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-3)', pointerEvents: 'none' }}><Icon name="search" size={14} /></span><input ref={inputRef} value={query} onChange={event => { setQuery(event.target.value); setOpen(true) }} onFocus={() => query.length >= 2 && setOpen(true)} onKeyDown={event => event.key === 'Escape' && setOpen(false)} placeholder="Buscar producto por código, nombre o ID Marco... (mín. 2 caracteres)" style={{ ...inputStyle, padding: '9px 10px 9px 32px', fontSize: 13 }} /></div>
    {open && query.length >= 2 && <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 200, maxHeight: 260, overflowY: 'auto', background: '#fff', border: '1px solid var(--border)', borderRadius: 8, boxShadow: 'var(--shadow-md)' }}>{productos.slice(0, 15).map(producto => <button key={producto.id} type="button" onClick={() => select(producto)} onMouseEnter={event => { event.currentTarget.style.background = 'var(--green-50)' }} onMouseLeave={event => { event.currentTarget.style.background = 'none' }} style={{ display: 'flex', width: '100%', gap: 10, padding: '9px 14px', alignItems: 'center', textAlign: 'left', background: 'none', border: 'none', borderBottom: '1px solid var(--border)', cursor: 'pointer' }}><img src={producto.fotoUrl || PRODUCT_PLACEHOLDER_IMAGE} alt="" loading="lazy" onError={useProductPlaceholderOnError} style={{ width: 32, height: 32, objectFit: 'cover', borderRadius: 4, border: '1px solid var(--border)' }} /><span style={{ minWidth: 80, color: 'var(--text-3)', fontFamily: "'DM Mono',monospace", fontSize: 11 }}>{producto.codigoInterno || '—'}</span><span style={{ flex: 1, minWidth: 0 }}><span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13, fontWeight: 500 }}>{producto.nombre}</span><span style={{ display: 'block', marginTop: 1, color: 'var(--text-3)', fontSize: 11 }}>Stock: <strong style={{ color: producto.stock > 0 ? 'var(--green-600)' : 'var(--red)' }}>{producto.stock}</strong> · Lista: <strong>{fmt(producto.precioLista)}</strong></span></span></button>)}{!productos.length && <div style={{ padding: '12px 14px', color: 'var(--text-3)', fontSize: 13 }}>Sin resultados para “{query}”</div>}</div>}
  </div>
}
...
function Total({ label, value, strong }) { return <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderTop: strong ? '1px solid var(--border)' : 'none', fontWeight: strong ? 700 : 400 }}><span>{label}</span><span style={{ fontFamily: "'DM Mono',monospace" }}>{fmt(value)}</span></div> }
