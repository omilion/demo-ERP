...
describeDb('ventas gerenciales: un negocio se cuenta una vez', () => {
  ...
  const creado = { ordenes: [], licitaciones: [], productoId: null }
  ...
  beforeAll(async () => {
    ...
    producto = await app.prisma.producto.create({
      data: {
        codigoInterno: `DEDUP-PROD-${Date.now()}`,
        nombre: 'Producto de prueba deduplicacion gerencial',
        activo: true,
        bodega: 'Inventario',
  ...
  afterAll(async () => {
    ...
    if (creado.productoId) await app.prisma.producto.deleteMany({ where: { id: creado.productoId } }).catch(() => {})
    await app.close()
  })
