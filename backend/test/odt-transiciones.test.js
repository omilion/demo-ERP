...
describeDb('la puerta de cierre de una OT', () => {
  ...
  async function otConEtapas(estados) {
    ...
    const odt = await app.prisma.odt.create({
      data: { centroCostoId: centro.id, clienteNombre: marca, estado: 'Control calidad' },
    })
  ...
  const cerrar = odtId => app.inject({
    ...
    payload: { estado: 'Terminada', controlCalidad: { aprobada: true, observacion: 'Control de calidad conforme' } },
  })
